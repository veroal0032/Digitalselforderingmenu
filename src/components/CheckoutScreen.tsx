...
export function CheckoutScreen({
  ...
  const subtotal = cartWithProducts.reduce((sum, item) => {
    ...
    if (item.size === 'large' && settings) {
      itemPrice += settings.large_size_extra;
    }
    return sum + itemPrice * item.quantity;
  }, 0);

  const extrasTotal = settings
    ? (orderExtras.collagen ? settings.extra_collagen_price : 0) +
      (orderExtras.ashwagandha ? settings.extra_ashwagandha_price : 0) +
      (orderExtras.honey ? settings.extra_honey_price : 0)
    : 0;
          ...
          >
            ...
            <div className="bg-[#F8F9F5] rounded-2xl p-6 space-y-4">
              {cartWithProducts.map((item) => {
                ...
                const productInfo = t.products[item.product.name_key as keyof typeof t.products];
                const itemPrice =
                  item.product.price +
                  (item.size === 'large' && settings ? settings.large_size_extra : 0);
              ...
              {/* Extras */}
              {orderData.extras_total > 0 && settings && (
                <>
                  <div className="border-t border-gray-300 pt-4">
                    ...
                    {orderExtras.collagen && (
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-sans-brand text-gray-600">
                          +${settings.extra_collagen_price.toFixed(2)}
                        </span>
                      </div>
                    )}
                    {orderExtras.ashwagandha && (
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-sans-brand text-gray-600">
                          +${settings.extra_ashwagandha_price.toFixed(2)}
                        </span>
                      </div>
                    )}
                    {orderExtras.honey && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-sans-brand text-gray-600">
                          +${settings.extra_honey_price.toFixed(2)}
                        </span>
                      </div>
                    )}
                  </div>
